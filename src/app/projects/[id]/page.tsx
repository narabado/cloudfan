...
interface Tier { id: string; name: string; amount: number; description: string; limit: number | null; remaining: number | null; }
interface Supporter { id: string; name: string | null; is_anonymous: boolean; amount: number; message: string | null; created_at: string; }
interface Project {
...
export default function ProjectDetail() {
  ...
  const [supporterCountByTier, setSupporterCountByTier] = useState<Record<string, number>>({});
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const resolvedTiers: Tier[] = (tierRows && tierRows.length > 0)
        ? tierRows as Tier[]
        : (Array.isArray(proj.tiers) ? proj.tiers as Tier[] : []);
      ...
      if (supRows) {
        ...
        const total = supRows.reduce((sum: number, r: { amount: unknown }) => sum + (Number(r.amount) || 0), 0);
        setTotalRaised(total);

        // ティアごとの支援者数カウント（tier_id があれば）
        const countMap: Record<string, number> = {};
        for (const s of supRows as Array<{ tier_id?: string; amount: unknown }>) {
          if (s.tier_id) countMap[s.tier_id] = (countMap[s.tier_id] || 0) + 1;
        }
        setSupporterCountByTier(countMap);
      }
      setLoading(false);
    })();
  }, [id]);
  ...
  const imgs = Array.isArray(project.images) ? project.images.filter(u => typeof u === 'string' && u.startsWith('http')) : [];
  ...
  // 上位ティア（支援者数順 → 金額順）最大5件、最低3件
  const rankedTiers = [...tiers].sort((a, b) => {
    const ca = supporterCountByTier[a.id] || 0;
    const cb = supporterCountByTier[b.id] || 0;
    if (cb !== ca) return cb - ca;
    return b.amount - a.amount;
  });
  const topTierCount = rankedTiers.length <= 3 ? rankedTiers.length : (supporterCountByTier[rankedTiers[2]?.id] === 0 ? Math.min(5, rankedTiers.length) : 3);
  const topTiers = rankedTiers.slice(0, topTierCount);
      ...
      <div style={{ fontFamily: "'Noto Sans JP', sans-serif", background: '#f8fafc', minHeight: '100vh' }}>
        ...
        <nav style={{ background: '#1a2e4a', padding: '12px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <Link href="/projects" style={{ color: 'rgba(255,255,255,0.85)', padding: '8px 16px', fontSize: 14 }}>支援する</Link>
            <Link href="/admin" style={{ color: 'rgba(255,255,255,0.85)', padding: '8px 16px', fontSize: 13 }}>管理</Link>
          </div>
        </nav>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px', display: 'grid', gridTemplateColumns: '1fr 340px', gap: 32 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
              <button onClick={copyUrl} style={{ padding: '6px 16px', background: copied ? '#059669' : '#1a2e4a', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>
            ...
            <div style={{ display: 'flex', gap: 4, borderBottom: '2px solid #1a2e4a', marginBottom: 0 }}>
              {[['story', '📖 ストーリー'], ['tiers', '🎁 支援プラン'], ['supporters', '👥 支援者']].map(([key, label]) => (
                <button key={key} onClick={() => setActiveTab(key as 'story' | 'tiers' | 'supporters')} style={tabStyle(key)}>{label}</button>
              ))}
            </div>
                ...
                <div>
                  ...
                  {storyBlocks.map((block, i) => (
                    <div key={i} style={{
                      ...
                      borderBottom: i < storyBlocks.length - 1 ? '2px dashed #e2e8f0' : 'none',
                    }}>
                      <h3 style={{
                        fontFamily: "'Noto Serif JP', serif",
                        background: '#1a2e4a', color: '#fff',
                        padding: '10px 18px', borderRadius: 8, marginBottom: 20, fontSize: 17,
                      }}>
                  ...
                  {ytId && (
                    <div style={{ marginTop: 24 }}>
                      ...
                      <div style={{ position: 'relative', paddingBottom: '56.25%', height: 0 }}>
                        <iframe src={`https://www.youtube.com/embed/${ytId}`} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', borderRadius: 8 }} allowFullScreen />
                      </div>
                    </div>
                  )}
                </div>
                      ...
                      <div key={tier.id} style={{
                        ...
                        <div style={{ padding: '16px 18px', background: '#fff' }}>
                          ...
                          <div style={{ display: 'flex', gap: 16, fontSize: 12, color: '#64748b', marginBottom: 16 }}>
                            ...
                            {tier.limit && <span>📦 残り {tier.remaining ?? tier.limit} 枠</span>}
                          </div>
                          <button style={{
                            width: '100%', padding: '11px', background: c, color: '#fff',
                    ...
                    <div key={s.id} style={{ display: 'flex', gap: 12, padding: '12px 0', borderBottom: '1px solid #f1f5f9', alignItems: 'flex-start' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ fontWeight: 700, color: '#1a2e4a', fontSize: 14 }}>{s.is_anonymous ? '匿名の支援者' : (s.name || '名前未設定')}</span>
                          <span style={{ color: '#2563eb', fontWeight: 700, fontSize: 15 }}>¥{(Number(s.amount) || 0).toLocaleString()}</span>
                        </div>
                        {s.message && <p style={{ color: '#64748b', fontSize: 13, margin: '4px 0 0', lineHeight: 1.7 }}>{s.message}</p>}
                        <span style={{ color: '#94a3b8', fontSize: 11 }}>{new Date(s.created_at).toLocaleDateString('ja-JP')}</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div>
            <div style={{ background: '#fff', borderRadius: 16, padding: 24, boxShadow: '0 4px 20px rgba(0,0,0,0.1)', position: 'sticky', top: 24 }}>
              ...
              <div style={{ textAlign: 'center', marginBottom: 16 }}>
                <span style={{
                  padding: '6px 20px', borderRadius: 20, fontWeight: 800, fontSize: 14,
                  background: statusBadge.bg, color: statusBadge.color,
                }}>
                  {statusBadge.label}
                </span>
              ...
              <button
                ...
                style={{
                  ...
                  background: project.status === '終了' ? '#94a3b8' : 'linear-gradient(135deg, #2563eb, #1a2e4a)',
                  color: '#fff', border: 'none', borderRadius: 10,
                  fontWeight: 800, fontSize: 16, cursor: project.status === '終了' ? 'not-allowed' : 'pointer',
                }}>
                {project.status === '終了' ? '募集終了' : '🏸 支援する'}
              </button>

              <button onClick={() => setActiveTab('tiers')}
                style={{ width: '100%', padding: '10px', background: '#f8fafc', color: '#2563eb', border: '2px solid #2563eb', borderRadius: 10, fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
                支援プランを見る
              </button>
