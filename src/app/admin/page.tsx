...
export default function AdminPage() {
  ...
  const filtered = supporters.filter(s => {
    const p = filterPrj==="all" || s.project_id===filterPrj;
    const st =
      filterSts==="all"       ? true :
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f8fafc",fontFamily:"sans-serif"}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"32px 16px"}}>
        {loading ? (
          <div style={{textAlign:"center",padding:80,fontSize:18,color:"#6b7280"}}>読み込み中...</div>
        ) : (
          <>
                            ...
                            <td style={{padding:"10px 12px"}}>
                              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                                ...
                                {!isCancelled(s.status) && (
                                  <button onClick={()=>{if(confirm("取り消しますか？"))updateStatus(s.id,"cancelled");}} style={{
                                    padding:"4px 10px",background:"#6b7280",color:"#fff",
                                    border:"none",borderRadius:6,cursor:"pointer",fontSize:12,
                                  }}>取消</button>
                                )}
                              </div>
            ...
            {tab==="projects" && (
              <div>
                <div style={{display:"flex",justifyContent:"flex-end",marginBottom:16}}>
                  <button onClick={()=>router.push("/admin/project-edit")} style={{
                    padding:"10px 24px",
                    background:"linear-gradient(135deg,#1e3a5f,#2d6a4f)",
                    color:"#fff", border:"none", borderRadius:8,
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:16}}>
                  {projects.map(p=>(
                    <div key={p.id} style={{
                      ...
                      <button onClick={()=>router.push(`/admin/project-edit?id=${p.id}`)} style={{
