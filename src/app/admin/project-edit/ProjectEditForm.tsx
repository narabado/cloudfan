...
export default function ProjectEditForm() {
  ...
  const addTier = () =>
    setTiers(ts => [...ts, { name: "", amount: 0, description: "" }]);
      ...
      <div style={{ maxWidth:"760px", margin:"0 auto" }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={cardStyle}>
            ...
            <p style={{ color:"#888", fontSize:"0.83rem", marginBottom:"1.2rem" }}>
              各ティアの名称・最低支援金額・特典内容を編集できます
            </p>

            {tiers.map((tier, i) => (
              <div key={i} style={{
                border: "1.5px solid #e0e8f0", borderRadius: "10px",
                padding: "1rem", marginBottom: "0.9rem", background: "#fafbfd",
                position: "relative",
              }}>
                <div style={{ position:"absolute", top:"0.8rem", right:"0.8rem" }}>
                  <button type="button" onClick={() => removeTier(i)}
                    style={{ background:"#fee2e2", border:"none", borderRadius:"6px",
                      padding:"0.25rem 0.6rem", color:"#dc2626", cursor:"pointer",
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0.8rem", marginBottom:"0.7rem" }}>
                  <div>
                    ...
                    <input style={inputStyle} value={tier.name}
                      onChange={e => setTier(i, "name", e.target.value)}
                      placeholder="例：ブロンズ" />
                  </div>
                  <div>
                    <label style={labelStyle}>最低支援金額（円）</label>
                    <input style={inputStyle} type="number" value={tier.amount}
                ...
                <div>
                  ...
                  <input style={inputStyle} value={tier.description}
                    onChange={e => setTier(i, "description", e.target.value)}
                    placeholder="例：お礼メール＋活動報告レポート送付" />
                </div>
              </div>
            ))}

            <button type="button" onClick={addTier}
              style={{ width:"100%", padding:"0.75rem", background:"#f0f4fa",
                border:"2px dashed #b0c4de", borderRadius:"10px",
                color:"#1a3a5c", cursor:"pointer", fontWeight:700, fontSize:"0.9rem" }}>
              ＋ ティアを追加
            </button>
          </div>
