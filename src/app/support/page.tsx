...
// ★ ブロンズを¥1,000に変更
const TIERS = [
  { name: "ブロンズ",   amount: 1000,   color: "#cd7f32", icon: "🥉", desc: "お礼メール＋活動報告レポート送付" },
  { name: "シルバー",   amount: 3000,   color: "#aaa",    icon: "🥈", desc: "＋オリジナルステッカー" },
  { name: "ゴールド",   amount: 10000,  color: "#d4af37", icon: "🥇", desc: "＋直筆サイン色紙" },
  { name: "プラチナ",   amount: 30000,  color: "#5be",    icon: "💎", desc: "＋練習見学招待＋記念写真撮影" },
  { name: "レジェンド", amount: 100000, color: "#e55",    icon: "👑", desc: "＋ユニフォームに名前掲載" },
];
...
function SupportPageInner() {
  ...
  const initTier = TIERS.find(t => t.name === tierParam) ?? TIERS[0];
  ...
  const [transferCode, setTransferCode] = useState("");   // ★ APIから受け取る
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/send-email", {
        ...
        body: JSON.stringify({
          supporterName:  name.trim(),       // ★ 既存APIに合わせたキー名
          supporterEmail: email.trim(),      // ★ 既存APIに合わせたキー名
    ...
    <>
      ...
      <div style={{ fontFamily: "sans-serif", minHeight: "100vh", background: "#f5f7fa" }}>
        ...
        <nav className="support-nav">
          ...
          <Link href="/projects/1" style={{ color: "#aaa", fontSize: 13, textDecoration: "none" }}>← プロジェクトへ</Link>
        </nav>
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "32px 16px" }}>
            ...
            <div style={{ background:"#fff", borderRadius:14, padding:24, boxShadow:"0 2px 12px rgba(0,0,0,.08)" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:24 }}>
                {TIERS.map(t => (
                  <div
                    key={t.name}
                    className={`tier-option ${selectedTier.name === t.name ? "selected" : ""}`}
              ...
              <div className="form-group">
                <label className="form-label">
                  ...
                  <span style={{ fontSize:12, color:"#888", fontWeight:"normal", marginLeft:8 }}>
                    ※複数口で自動ティアアップ
                  </span>
                </label>
                    ...
                    if (need <= 10 && need > units) {
                      return (
                        <div className="upgrade-notice">
                          ⬆️ あと{need - units}口増やすと
                          <strong>{next.icon} {next.name}</strong>に自動アップグレード！
                        </div>
                      );
                    }
                  }
            ...
            <div style={{ background:"#fff", borderRadius:14, padding:24, boxShadow:"0 2px 12px rgba(0,0,0,.08)" }}>
              ...
              <div style={{ background:"#f5f7fa", borderRadius:8, padding:14, marginBottom:20, fontSize:14 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                  ...
                  <span style={{ fontWeight:"bold", color:selectedTier.color }}>
                    {selectedTier.icon} {selectedTier.name}（{units}口）
                  </span>
                </div>
                <div style={{ display:"flex", justifyContent:"space-between" }}>
          ...
          {step === "done" && (
            <div>
              ...
              <Link href="/projects/1" style={{ display:"block", background:"linear-gradient(135deg,#d4af37,#f0c040)", color:"#0a1628", textAlign:"center", borderRadius:10, padding:15, fontWeight:"bold", fontSize:16, textDecoration:"none", marginBottom:12 }}>
                プロジェクトページへ戻る
              </Link>
              <Link href="/" style={{ display:"block", background:"transparent", color:"#555", textAlign:"center", borderRadius:10, padding:13, fontWeight:"bold", fontSize:15, textDecoration:"none", border:"2px solid #ddd" }}>
                トップページへ
              </Link>
            </div>
          )}
